...
async function answer(query, apiConfig, cp = false, obj, model) {
    ...
    if (cp || !obj?.db?.dbPath) {
        prompt = cp ? query : "You are a helpful voice assistant named Bumblebee made by Hridhuun Savant. Answer the user's question concisely in one or two sentences. Avoid markdown; output plain text only. This text is going to be parsed into a tts tool, so keep it easy to read. Here is the query: " + query;
    } else {
        const results = await obj.db.searchDB(query, 10, obj.table_config);
        context = results.map(item => `${item.text} (similarity score: ${item.similarity})`).join("\n");
    ...
    if (workbench && !cp) {
        prompt = "You are Bumblebee, a helpful voice assistant made by Hridhuun Savant. You are operating in workbench mode. Provide complete, detailed responses. Do not truncate. Use markdown formatting including headers, code blocks, and lists where appropriate. When asked to genreate code, generate full codes, with valid markdown codeblocks of that particular language.Avoid emojis unless the user specifies it, or its needed.\n\n\nRetrieved Context:\n" + context + "\nQuery: " + query;
    }
